...
//  #define FORMANT_TRACE 1   // Only enable to printout random formants in simulator
...
int lo=0; int hi=0;
  ...
  for( int i=lo; i <= hi; i++)   // Process all 5 formants
  {
    ...
    // ### ----------------
#ifdef FORMANT_TRACE   // ### debugging in simulator, delete later!
    fprintf(log_ptr,"%d) Cut,Res,Amp: %f, %f, %f, %f, %f, %f, %f, %f, %f\n", formant_counter,  f_CutOffXarray[i], f_CutOffYarray[i], f_CutOffZarray[i],f_ResoXarray[i], f_ResoYarray[i], f_ResoZarray[i],f_FltAmntXarray[i], f_FltAmntYarray[i], f_FltAmntZarray[i]);
    formant_counter++;
    fflush(log_ptr); // ###
#endif
    // ### ----------------
...
bool b_use_fix_formants = true;
  ...
  if( t_KeyLogic )    // Select formants via notes on a keyboard or the slider on the GUI
    formant_selected = i_current_note%12; // i_current_note%12;      // ###  // We have max 12 formants, one associated to every key, regardless of its octave
  else    // No key logic
  ...
  for(uint32_t i = 0; i < bufSz; i++)
  {
    ...
    f_val_result = f_right_direct_out = f_rescomb_process = (f_val_pd*f_PDvol+f_val_sqw*f_SQWvol) / 2.f;    // Check if we already devide here? ###
...
ctagSoundProcessorFormantor::ctagSoundProcessorFormantor()
{
  ...
  // ###
#ifdef FORMANT_TRACE
  log_ptr = fopen ( "D:/tmp/formant_log.txt", "w" );
#endif
  // ###
