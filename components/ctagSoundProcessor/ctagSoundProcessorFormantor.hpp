...
namespace CTAG {
    namespace SP {
        ...
        private:
            ...
            // TBDdejitter cvPitch = TBDdejitter();     // Average 10 concurrent reading for CV! (Default is 10)
            ...
            // --- VULT Stuff ---
            vult::Phasedist_real_process_type pd_data;        // VULT PD synth voice internal datastructure, also needed for initialisation
            vult::Svf__ctx_type_4 svf_data_x;                 // State Variable Filter (Bandpass) init (svf_data_x,y,z... for 3 bandpasses);
            vult::Svf__ctx_type_4 svf_data_y;
            vult::Svf__ctx_type_4 svf_data_z;
            vult::Rescomb__ctx_type_6 rescomb_data;           // Resonator (comb-filter) data-structure
            ...
            // ### -----------------
#ifdef FORMANT_TRACE
            bool my_toggle_rnd = true;
            int my_counter = 0;
            FILE* log_ptr = NULL;
            unsigned int formant_counter = 0;
#endif
            // ### --------------------
